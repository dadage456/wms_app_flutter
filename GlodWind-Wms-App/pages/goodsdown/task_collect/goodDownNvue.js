...
export default {
	...
	methods: {
		...
		getList() {
			...
			console.log('-------------------采集列表页面数据请求-----------------');
			console.log('接口请求：getOutTaskCollitemList');
			console.log('请求参数:', this.outTaskItem);
			getOutTaskCollitemList(this.outTaskItem).then((response) => {
				...
				if (response.code == '200') {
					console.log('response数量' + response.data.length);
					this.detailListView = response.data;
					console.log('返回数据：', response);
					console.log('列表加载');
					console.log('接口请求：GetRoomMatControl');
					console.log('请求参数:', taskId);
					this.GetRoomMatControl(taskId).then((response5) => {
						console.log("返回数据:", response5);
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							...
							console.log('-----------采集提交操作------------------');
							console.log('采集的数据:',collectStocks);
							console.log('提交参数', downShelvesInfosList);
		...
		subClick(e) {
			...
			if (e.index === 1) {
				...
				uni.$off('scancodedate');
				setTimeout(() => {
					console.log('/pages/exceptTask/exceptTask?taskComment=' +
							taskComment +
							'&taskNo=' +
							taskNo +
							'&taskid=' +
