...
/// 出库采集BLoC
class OutboundCollectBloc
    extends Bloc<OutboundCollectEvent, OutboundCollectState> {
  final OutboundTaskService _outboundTaskService;

  OutboundCollectBloc(this._outboundTaskService)
    : super(const OutboundCollectState.initial()) {
  ...
  Future<void> _onSearchTaskItems(
    ...
    try {
      ...
      emit(currentState.copyWith(errorMessage: '搜索任务明细失败: $e'));
    }
  }
  ...
  Future<void> _onProcessScanCode(
    ...
    emit(
      OutboundCollectState.scanProcessing(
    ...
    try {
      ...
      switch (currentState.currentScanStep) {
        ...
        case ScanStep.material:
          // 解析物料条码
          final barcodeContent = await _outboundTaskService.parseBarcodeContent(
            event.content,
          );
          if (barcodeContent != null) {
            add(
              OutboundCollectEvent.validateMaterial(
                barcodeContent: barcodeContent,
                siteCode: currentState.currentSiteCode ?? '',
              ),
            );
          } else {
            emit(currentState.copyWith(errorMessage: '无法解析条码内容'));
          }
          break;
      ...
      emit(currentState.copyWith(errorMessage: '处理扫码内容失败: $e'));
    }
  }
  ...
  Future<void> _onValidateSite(
    ...
    try {
      ...
      if (hasValidSite) {
        ...
        emit(currentState.copyWith(errorMessage: '库位不在当前任务中'));
      }
    } catch (e) {
      emit(currentState.copyWith(errorMessage: '验证库位失败: $e'));
    }
  }
  ...
  Future<void> _onValidateMaterial(
    ...
    try {
      // 检查物料是否在当前库位的任务明细中
      final taskItem = currentState.taskItems
          .where(
            (item) =>
                item.storeSiteNo == event.siteCode &&
      ...
      if (taskItem != null) {
        ...
        emit(currentState.copyWith(errorMessage: '物料不在当前库位的任务中'));
      }
    } catch (e) {
      emit(currentState.copyWith(errorMessage: '验证物料失败: $e'));
    }
  }
  ...
  Future<void> _onQueryInventory(
    ...
    emit(
      OutboundCollectState.inventoryQuerying(
        task: currentState.task,
        taskItems: currentState.taskItems,
        collectRecords: currentState.collectRecords,
        currentTabIndex: currentState.currentTabIndex,
    ...
    try {
      final response = await _outboundTaskService.getMtlRepertoryByStoresiteNo(
        query: event.query,
      );
      ...
      emit(currentState.copyWith(errorMessage: '库存查询失败: $e'));
    }
  }
  ...
  Future<void> _onConfirmQuantity(
    ...
    try {
      if (currentState.currentBarcodeContent != null &&
        ...
        emit(currentState.copyWith(errorMessage: '采集信息不完整'));
      }
    } catch (e) {
      emit(currentState.copyWith(errorMessage: '确认数量失败: $e'));
    }
  }
  ...
  Future<void> _onClearCollectRecords(
    ...
    emit(currentState.copyWith(collectRecords: [], successMessage: '采集记录已清空'));
  }
  ...
  Future<void> _onSubmitCollectData(
    ...
    if (currentState.collectRecords.isEmpty) {
      emit(currentState.copyWith(errorMessage: '没有采集记录可提交'));
      return;
    }

    emit(
      OutboundCollectState.submitting(
        task: currentState.task,
        taskItems: currentState.taskItems,
        collectRecords: currentState.collectRecords,
        currentTabIndex: currentState.currentTabIndex,
    ...
    try {
      ...
      if (response.code == '200') {
        emit(currentState.copyWith(collectRecords: [], successMessage: '提交成功'));

        // 刷新任务明细列表
        add(const OutboundCollectEvent.refreshTaskItems());
      } else {
        emit(currentState.copyWith(errorMessage: response.message ?? '提交失败'));
      }
    } catch (e) {
      emit(currentState.copyWith(errorMessage: '提交失败: $e'));
    }
  }
  ...
  Future<void> _onReportShortage(
    ...
    try {
      ...
      if (response.code == '200') {
        emit(currentState.copyWith(successMessage: '报缺成功'));

        // 刷新任务明细列表
        add(const OutboundCollectEvent.refreshTaskItems());
      } else {
        emit(currentState.copyWith(errorMessage: response.message ?? '报缺失败'));
      }
    } catch (e) {
      emit(currentState.copyWith(errorMessage: '报缺失败: $e'));
    }
  }
  ...
  Future<void> _onSetScanStep(
    ...
    emit(currentState.copyWith(currentScanStep: event.step));
  }
  ...
  Future<void> _onSetCurrentSite(
    ...
    emit(currentState.copyWith(currentSiteCode: event.siteCode));
  }
  ...
  Future<void> _onSetCurrentMaterial(
    ...
    emit(currentState.copyWith(currentBarcodeContent: event.barcodeContent));
  }
  ...
  Future<void> _onExceptionCollect(
    ...
    try {
      ...
      emit(currentState.copyWith(errorMessage: '异常采集失败: $e'));
    }
  }
}
