...
class OutboundCollectEvent with _$OutboundCollectEvent {
  ...
  /// 验证库位
  const factory OutboundCollectEvent.validateSite({required String siteCode}) =
      ValidateSiteEvent;
  ...
  /// 确认采集数量
  const factory OutboundCollectEvent.confirmQuantity({required int quantity}) =
  ...
  /// 删除采集记录
  const factory OutboundCollectEvent.removeCollectRecord({required int index}) =
      RemoveCollectRecordEvent;

  /// 清空采集记录
  const factory OutboundCollectEvent.clearCollectRecords() =
      ClearCollectRecordsEvent;

  /// 提交采集数据
  const factory OutboundCollectEvent.submitCollectData() =
      SubmitCollectDataEvent;
  ...
  /// 切换Tab
  const factory OutboundCollectEvent.switchTab({required int tabIndex}) =
      SwitchTabEvent;
  ...
  /// 设置当前扫码步骤
  const factory OutboundCollectEvent.setScanStep({required ScanStep step}) =
      SetScanStepEvent;
